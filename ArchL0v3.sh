...
echo ""
echo "--------- /dev/nvme0n1 - 16G will be mounted as SWAP space ------------"
echo "--------- /dev/nvme0n2 - Rest of space will be mounted as / -----------"
echo ""
...
sed -e 's/\s*\([\+0-9a-zA-Z]*\).*/\1/' << EOF | fdisk /dev/nvme0n1 # CHANGE THIS IF NEEDED!!!
...
curl -O https://raw.githubusercontent.com/ChaoticHackingNetwork/Scripts/master/ArchL0v3Final.sh
mv ACBI.sh /mnt
...
echo "	[2] chmod +x ArchL0v3Final.sh"
echo "  [3] ./ArchL0v3Final.sh"
